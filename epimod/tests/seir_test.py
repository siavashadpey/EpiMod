...
class TestSeir(unittest.TestCase):
	...
	def test_source(self):
		...
		self.assertAlmostEqual(np.sum(s), 0.0, delta)
		np.testing.assert_almost_equal(s, s_act, delta)
	...
	def test_source_state_grad(self):
		...
		for i in range(eqn.n_components()):
			...
			np.testing.assert_almost_equal(ds_du[:,i], (s_p1 - s_m1)/(2*epsi), delta)
	...
	def test_source_param_grad(self):
		...
		eqn = Seir(beta - epsi, sigma, gamma)
		s_m1 = eqn.source(u=u)
		np.testing.assert_almost_equal(ds_dp[:,0], (s_p1 - s_m1)/(2*epsi), delta)
		...
		eqn = Seir(beta, sigma - epsi, gamma)
		s_m1 = eqn.source(u=u)
		np.testing.assert_almost_equal(ds_dp[:,1], (s_p1 - s_m1)/(2*epsi), delta)
		...
		eqn = Seir(beta, sigma, gamma - epsi)
		s_m1 = eqn.source(u=u)
		np.testing.assert_almost_equal(ds_dp[:,2], (s_p1 - s_m1)/(2*epsi), delta)
	...
	def test_output(self):
		...
		self.assertAlmostEqual(f, u[2], delta)
	...
	def test_output_param_grad(self):
		...
		np.testing.assert_almost_equal(df_dp, du_dp[2,:], delta)
